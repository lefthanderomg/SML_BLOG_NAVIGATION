...
### Вложенный граф
Вложнный граф делается очень легко, нужно только выбрать экран с которого будет начинаться вложенный граф и установить для него новый граф.
...
И теперь мы можем переходить во вложенный граф из главного. При этом наглядно видно куда мы переходим, и что это не просто экран, а вложенный граф. При этом новый xml файл не создается, fragments просто заключаются в navigation тэг.
